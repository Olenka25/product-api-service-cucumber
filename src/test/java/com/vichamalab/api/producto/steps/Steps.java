...
public class Steps {
	...
	public void a_la_api() {
		if ("POST".equals(currentMethod.toUpperCase())) {
			...
			response = APIProducto.recuperarProducto(currentUrl, originalSku);
		}
	}
	...
	@Entonces("y el estado {string}")
	public void y_el_estado(String estado) {
		Assertions.assertEquals(estado,response.jsonPath().getString("status"));
	}
